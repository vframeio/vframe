...
from vframe.settings.app_cfg import LOG, VALID_PIPE_IMAGE_EXTS, VALID_PIPE_VIDEO_EXTS
...
class FileVideoStream:
            ...
            try:
                ...
                if file_utils.get_ext(fp).lower() in VALID_PIPE_IMAGE_EXTS:
                    ...
                    if check_idx != self.frame_count:
                        LOG.warn(
                            f"Partial video: rewinding from {self.frame_count} to: {check_idx}"
                        )
    ...
    def start(self):
        ...
        if self.width > 0 and self.height > 0 and self.frame_count > 0 and self.fps > 0:
    ...
    if media_type == "invalid":
        ...
        if video_attrs and general_attrs:
            ...
            if "encoded_date" in general_attrs.keys():
                ...
                created_at = str(
                    datetime.strptime(encoded_date, "%Z %Y-%m-%d %H:%M:%S")
                )
