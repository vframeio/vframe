            ...
            for i in range(n_a):
                ...
                for j in range(n_b):
                    ...
                    interp_coord = list(
                        zip(
                            np.linspace(
                                cand_a[i][0], cand_b[j][0], num=n_interp_samples
                            ),
                            np.linspace(
                                cand_a[i][1], cand_b[j][1], num=n_interp_samples
                            ),
                        )
                    )
                    ...
                    if found:
                        valid_pair = np.append(
                            valid_pair,
                            [[cand_a[i][3], cand_b[max_j][3], score_max]],
                            axis=0,
                        )
            ...
            for i in range(len(valid_pairs[k])):
                ...
                if found:
                    ...
                    personwise_keypoints[person_idx][-1] += (
                        keypoints_list[parts_b[i].astype(int), 2] + valid_pairs[k][i][2]
                    )
    ...
    for part in range(N_POINTS):
        ...
        for i in range(len(keypoints)):
            keypoints_with_id.append(
                keypoints[i] + (keypoint_id, KEYPOINTS_MAPPING[part])
            )
...
def keypoints_to_persons(detected_keypoints, personwise_keypoints):
    ...
    for n in range(n_persons):
        ...
        for i in range(17):
            kpts += (
                personwise_keypoints[n][np.array(POSE_PAIRS[i])].astype(int).tolist()
            )
        kpts = list(set(kpts))
        if -1 in kpts:
            kpts.pop(-1)
    ...
    for i in range(n_persons):
        pose = {
            detected_keypoints_lkup[x][2]: detected_keypoints_lkup[x][:2]
            for x in persons_kpt_idxs[i]
            if x != -1
        }
        persons.append(pose)
...
def pose_to_bbox(pose, dim):
    ...
    head_exists = (
        nose and neck and (shoulder_right or shoulder_left) and (eye_left or eye_right)
    )  # strict
    ...
    if ear_right:
        ...
        x1 = min(
            eye_right[0],
            eye_right[0] - (0.5 * max(0, abs(shoulder_right[0] - eye_right[0]))),
        )
    elif eye_right:
    ...
    if ear_left:
        ...
        x2 = max(
            eye_left[0],
            eye_left[0] + (0.5 * max(0, abs(shoulder_left[0] - eye_left[0]))),
        )
    elif eye_left:
        # double the distance between nose and eye
