...
@click.pass_context
def cli(ctx, opt_model_enum, opt_dl_all, opt_force):
  """Download DNN models"""
  ...
  if opt_model_enum:
    ...
    model_list = list(modelzoo_cfg.modelzoo.keys())
  else:
    app_cfg.LOG.error('No model(s) selected. Choose "-m/--model" or download "--all"')
    return
