...
def choices_description(plugins):
  ...
  clr_t: str = '\033[0m'
  sp_max: int = 30 + len(clr_h) + len(clr_t)
  t = ['Command options:']
...
argv_tmp = sys.argv
sys.argv = sys.argv[:2]
ap = argparse.ArgumentParser(usage="./cli.py [command]",
                             description="\033[1m\033[94mVFRAME CLI (beta)\033[0m",
                             formatter_class=argparse.RawDescriptionHelpFormatter,
                             epilog=choices_description(plugins_cfg.plugins.plugins))
...
for plugin_script in plugin_group.scripts:
    ...
    for fp_py in iglob(fp_dir_glob):
        ...
        fn = Path(fp_py).name
        if plugin_script.include_hidden is False and fn[0] == '_':
            continue
